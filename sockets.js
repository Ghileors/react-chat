...
module.exports = io => {
  ...
  io.on('connection', async socket => {
    ...
    const users = user.get('isAdmin')
      ? await UserModel.find()
      : await UserModel.find({ isOnline: true });
    ...
    if (user.isAdmin) {
      socket.on('ban', async userId => {
        ...
        UserModel.findByIdAndUpdate(
          userId,
          { isBanned: !bannedUser.get('isBanned'), isOnline: false },
          () => {},
        );
      });
